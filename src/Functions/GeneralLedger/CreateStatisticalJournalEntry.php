...
class CreateStatisticalJournalEntry extends AbstractGlBatch
{
    ...
    public function writeXml(XMLWriter &$xml)
    {
        ...
        if (count($this->getEntries()) >= 1) {
            foreach ($this->getEntries() as $entry) {
                if ($entry instanceof CreateStatisticalJournalEntryEntry) {
                    $entry->writeXml($xml);
                } elseif (is_array($entry)) {
                    $entry = new CreateStatisticalJournalEntryEntry($entry);

                    $entry->writeXml($xml);
                }
            }
        } else {
            throw new InvalidArgumentException('Statistical Journal Entry "entries" param must have at least 1 entry');
        }
